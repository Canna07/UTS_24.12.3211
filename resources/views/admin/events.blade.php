    ...
    <main class="flex-1 p-10 overflow-y-auto">
        <header class="flex justify-between items-center mb-10">
            ...
            <a href="{{ route('admin.events.create') }}"
                class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition inline-block">
                + Tambah Event Baru
            </a>
        </header>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    @forelse($events as $index => $event)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-8 py-6 w-16 h-20 overflow-hidden rounded-xl">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex gap-2">
                                    ...
                                    <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus event ini?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
